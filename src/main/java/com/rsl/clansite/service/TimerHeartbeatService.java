...
import java.util.concurrent.atomic.AtomicBoolean; // Important Import
...
public class TimerHeartbeatService {
    ...
    // Safety Lock: Prevents multiple heartbeat threads from stacking up if the network is slow
    ...
    public void updateDiscordHub() {
        // SAFETY CHECK 1: If the previous run is still going, skip this one.
        // compareAndSet(false, true) means: "If it is false, set it to true and return success. If already true, return fail."
        if (!isProcessing.compareAndSet(false, true)) {
            log.warn("Heartbeat skipped: previous pulse still in progress. Checking for thread stall.");
            return;
        }

        try {
            // SAFETY CHECK 2: Do not attempt to use JDA if the bot is not fully connected.
            // This prevents the "Startup Stampede" we saw earlier.
            if (jda == null || jda.getStatus() != JDA.Status.CONNECTED) {
                log.debug("Heartbeat skipped: JDA is not yet connected.");
                return;
            }
            ...
            embedBuilder.setTitle("🛡️ Clan - Strategic Timers");
            embedBuilder.setColor(new Color(41, 128, 185));
            ...
            log.error("Critical error in timer heartbeat loop", e);
        } finally {
            // SAFETY CHECK 3: Always release the lock so the next minute can run
